...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const activationResponse = await fetch(
        `http://localhost:3000/api/v1/activations/fae49ff3-6a4c-4e7a-9868-de340c61d698`,
        {
          method: "PATCH",
        },
      );
  ...
  describe("Default user", () => {
    test("With valid token, but already logged in user", async () => {
      ...
      const user1SessionsObject = await orchestrator.createSession(user1.id);

      const user2 = await orchestrator.createUser();
      const activationToken = activation.create(user2.id);
      ...
      const response = await fetch(
        `http://localhost:3000/api/v1/activations/${activationToken.id}`,
        {
          method: "PATCH",
          headers: {
            Cookie: `session_id=${user1SessionsObject.token}`,
          },
        },
      );
