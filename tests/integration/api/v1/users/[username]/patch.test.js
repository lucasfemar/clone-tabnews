...
describe("PATCH  /api/v1/users/[username]", () => {
  ...
  describe("Default user", () => {
    ...
    test("With case mismatch 'username'", async () => {
      const caseMismatchUser = await orchestrator.createUser({
        username: "caseUser",
      });
      const activatedUser = await orchestrator.activateUser(caseMismatchUser);
      const sessionObject = await orchestrator.createSession(activatedUser.id);

      const response = await fetch(
        `http://localhost:3000/api/v1/users/${caseMismatchUser.username}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
